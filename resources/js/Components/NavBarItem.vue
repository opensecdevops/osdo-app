...
const itemLabel = computed(() =>
  props.item.isCurrentUser && usePage().props.auth.user !== null  ? usePage().props.auth.user.name : props.item.label
)

const isLogged = usePage().props.auth.user

const drawMenu = computed(() => {
  if (props.item.menu && typeof props.item.isCurrentUser === 'undefined') {
    return true
  } 
  if (props.item.menu && props.item.isCurrentUser && isLogged != null) {
    return true
  }
  return false
});
...
<template>
  ...
  >
    ...
    >
      ...
      <UserAvatarCurrentUser v-if="drawMenu" class="w-6 h-6 mr-3 inline-flex" />
    ...
    >
      <NavBarMenuList v-if="drawMenu" :menu="item.menu" @menu-click="menuClickDropdown" />
    </div>
  </component>
</template>
